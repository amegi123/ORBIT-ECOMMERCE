...
export const MobileStickyBar: React.FC<MobileStickyBarProps> = ({ product }) => {
  ...
  return (
    <div className="lg:hidden fixed bottom-[56px] left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-slate-200 p-3 px-4 shadow-2xl flex items-center justify-between gap-3 transform-gpu translate-z-0">
      <div>
      ...
      <div className="flex items-center gap-2 flex-1 max-w-xs">
        ...
        <button
          onClick={() => addToCart(product)}
          className="flex-1 bg-amber-400 hover:bg-amber-500 text-slate-950 text-xs font-extrabold py-2.5 px-3 rounded-xl flex items-center justify-center shadow"
        >
          <span>Buy Now</span>
        </button>
      </div>
