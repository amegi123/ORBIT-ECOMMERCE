    ...
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="fixed inset-y-0 right-0 max-w-full flex pl-10">
        <div className="w-screen max-w-md bg-white shadow-2xl flex flex-col justify-between animate-in slide-in-from-right duration-300">
          {/* Drawer Header */}
          <div className="p-5 border-b border-slate-200 flex items-center justify-between bg-[#0B3C80] text-white">
            <div className="flex items-center gap-2">
              <ShoppingBag className="w-5 h-5 text-amber-400" />
              <h3 className="font-bold text-base">Your Shopping Cart ({cart.length})</h3>
            </div>
            <button
              onClick={() => setIsCartOpen(false)}
            ...
            {cart.length === 0 ? (
              ...
              cart.map((item, idx) => (
                <div key={idx} className="pt-4 first:pt-0 flex gap-4 items-center">
                  <div className="relative w-20 h-20 bg-slate-50 rounded-xl border border-slate-200 shrink-0 p-1">
                    <Image
            ...
            <div className="p-5 border-t border-slate-200 bg-slate-50 space-y-4">
              <div className="space-y-1 text-xs">
                ...
                <div className="flex justify-between text-sm font-black text-slate-950 pt-2 border-t border-slate-200">
                  ...
                  <span className="text-blue-600">{subtotal.toLocaleString()} ETB</span>
                </div>
              </div>
